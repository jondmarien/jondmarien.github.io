...
foreach ($file in $markdownFiles) {
    ...
    try {
        ...
        foreach ($match in $matches) {
            ...
            # Skip if it already starts with Resources/ (already partially corrected)
            if ($imageName.StartsWith('Resources/')) {
                Write-Host "  Found already corrected path: $imageName in $($file.Name)" -ForegroundColor Cyan
                # Convert to absolute path
                $newImageRef = "![$altText](/$imageName)"
            ...
            try {
                $foundImage = Get-ChildItem -Path "content/Resources" -Filter $imageName -Recurse -File -ErrorAction SilentlyContinue | Select-Object -First 1
                
                if (-not $foundImage) {
                    # Try URL decoding the filename
                    $decodedName = [System.Web.HttpUtility]::UrlDecode($imageName)
                    $foundImage = Get-ChildItem -Path "content/Resources" -Filter $decodedName -Recurse -File -ErrorAction SilentlyContinue | Select-Object -First 1
                }
            }
            catch {
                Write-Host "    ⚠️ Error searching for image: $($_.Exception.Message)" -ForegroundColor Red
            }
...
if ($errorFiles -gt 0) {
    Write-Host ""
    Write-Host "⚠️  Some files had errors. You may want to check them manually." -ForegroundColor Yellow
}